...
- [ ] **1.4.2 performance bugfix:** the Belgium project load regression is fixed by deferring hidden placement work, caching centroids, and lazily rebuilding logistics overlay colors. The measured load path fell from about 51 seconds to about 1.3 seconds; full release/build evidence will be recorded after the package is produced.
...
- **Release evidence:** v1.4.1 was built from source commit `2986b8b`, smoke-tested, checksummed, and published with a human-oriented description that links to `CHANGELOG.md`.
